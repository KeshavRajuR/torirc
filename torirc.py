...
# Random wait before sending the "noise message" to the server
...
class torStem():
    ...
    def connect(self, addr='127.0.0.1', cport=9051):
        ...
        print("[I] Tor relay is alive. %s bytes read, %s bytes written." %
        ...
        self.controller.set_options([
            ...
            ('HiddenServicePort', "%d %s:%d" %
             (hidden_service_port, hidden_service_interface, hidden_service_port))
        ])
        self.hostname = open(
            "%s/hostname" % newHiddenServiceDir, "rb").read().strip()
...
class Server():
            ...
            try:
                ...
                if ready[0]:
                    ...
                    if data.startswith("/PING"):
                        ...
                        msg.append(data)
                        continue
                    # Change nick. Note that we do not add to roster before
                    # this operation
    ...
    def serverMain(self, channel_name):
        ...
        log('[I] Server Active')
        log('[I] Connect with the command "%s --connect=%s"' %
            (sys.argv[0], self.ts.hostname))
        s.listen(5)
        ...
        while True:
            try:
                ...
                cmsg.append("Welcome %s, this is %s" %
                            (nick, self.channelname))
                self.servermsgs.append(cmsg)
                t = Thread(
                    target=self.serverThread, args=(conn, addr, cmsg, nick))
                t.daemon = True
                t.start()
...
def chat_server_help(args):
    return "/serverhelp"
commands.append(
    ("serverhelp", chat_server_help, "Request sever commands help"))
...
def clientConnectionThread(stdscr, ServerOnionURL, msgs):
    ...
    while(True):
        try:
            ...
            s.setproxy(
                socks.PROXY_TYPE_SOCKS5, tor_server, tor_server_socks_port)
            s.settimeout(100)
...
def clientMain(stdscr, ServerOnionURL):
    ...
    msgs = []
    t = Thread(target=clientConnectionThread, args=(
        stdscr, ServerOnionURL, msgs))
    t.daemon = True
    t.start()
...
if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("-c", "--connect", action="store", type="string",
                      dest="connect", help="Acts as client, connect to server")
    parser.add_option("-s", "--server", action="store",
                      type="string", dest="channel_name",
                      help="Acts as server")
    # no arguments->bail
    if len(sys.argv) == 1:
        parser.print_help()

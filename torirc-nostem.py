...
def initTor():
    ...
    try:
        ...
        if tor_pid:
            ...
            while cnt <= 20:
                try:
                    log("(1) trying to read hostname file (try %i of 20)" %
                        (cnt + 1))
            ...
            if not found:
                ...
                log(
                    "(1) switching active config section from [tor] to [tor_portable]")
...
class Server():
    ...
    def serverMain(self, channel_name):
        ...
        log("(Main Server Thread) Tor looks active, listening on %s" %
            hostname)
        s.listen(5)
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        type="string",
        dest="connect",
        help="Acts as client, connect to server")
    parser.add_option(
        "-s",
        "--server",
        action="store",
